...
def send_document_clean_notify_task():
    ...
    logging.info(click.style(
        "Start send document clean notify mail", fg="green"))
    start_at = time.perf_counter()

    # send document clean notify mail
    try:
        dataset_auto_disable_logs = DatasetAutoDisableLog.query.filter(
            DatasetAutoDisableLog.notified == False).all()
        # group by tenant_id
        dataset_auto_disable_logs_map: dict[str,
                                            list[DatasetAutoDisableLog]] = defaultdict(list)
        for dataset_auto_disable_log in dataset_auto_disable_logs:
            ...
            dataset_auto_disable_logs_map[dataset_auto_disable_log.tenant_id].append(
        ...
        for tenant_id, tenant_dataset_auto_disable_logs in dataset_auto_disable_logs_map.items():
            ...
            if plan != "sandbox":
                ...
                account = Account.query.filter(
                    Account.id == current_owner_join.account_id).first()
                if not account:
                    continue
                ...
                for dataset_id, document_ids in dataset_auto_dataset_map.items():
                    ...
                    if dataset:
                        ...
                        knowledge_details.append(
                            rf"Knowledge base {dataset.name}: {document_count} documents")
                if knowledge_details:
                    ...
                    mail.send(
                        to=account.email, subject="DoyelAI Knowledge base auto disable notification", html=html_content
                    )
        ...
        logging.info(
            click.style("Send document clean notify mail succeeded: latency: {}".format(
                end_at - start_at), fg="green")
        )
    except Exception:
